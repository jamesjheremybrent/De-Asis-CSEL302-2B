...
species student {
    ...
    int low_attention_cycles <- 0;   // Add for option C fatigue model
    ...
    reflex update_attention {
        ...
        // Add for option C fatigue model
        if (attention < 0.3) {
            low_attention_cycles <- low_attention_cycles + 1;
        } else {
            low_attention_cycles <- 0;
        }

        if (low_attention_cycles >= 10) {
            performance <- max(0.0, performance - 0.02);
        }
